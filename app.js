    ...
    wx.getNetworkType({
      success: function (res) {
        ...
        if (res.networkType == "none") { //无网络
          ...
          that.http.get({
            url: that._server + "/api/config"
          }).then(data => {
            that.init(data);
          }).catch(error => {
            that.offline = true; //离线模式
          })
        }
      },
    });
